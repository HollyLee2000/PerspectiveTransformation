...
class ImagePerspective:
    def __init__(self,):            
        ...
        self.root.title('图片透视变换')
        self.button = tkinter.Button(self.root, text="选择图像", command=self.get_files,width=20,height=2)
        ...
        self.button4 = tkinter.Button(self.root, text="透视变换-默认布满画布", command=self.pic_perspective_auto,width=20,height=2)
        self.button4.grid(row=2, column=0, padx=1, pady=1)
        self.root.geometry('500x220+600+300')
        self.root.mainloop()
    ...
    def mouse(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            pts1.append([x, y])
            xy = "%d,%d" % (x, y)
            # tkinter.messagebox.showinfo('提示', '你选择了坐标为（'+str(x)+', '+str(y)+'）的点!')
    ...
    def pic_perspective_auto(self,):    
        ...
        cv2.waitKey(0)
        cv2.destroyAllWindows()
        pts2.append(top_left)
        pts2.append(top_right)
        pts2.append(bottom_left)
        pts2.append(bottom_right)
        print("pts1:", pts1)
        pts1 = np.float32(pts1[:4])
        print("pts2:", pts2)
        pts2 = np.float32(pts2[:4])
